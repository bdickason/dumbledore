...
describe('Commends', function() {
	...
	describe('Chat commands', function() {
		it('Contains settings for !commends', function() {
			assert.notEqual(Commends.commands, null);
			assert.equal(Commends.commands[0].name, 'commends');
			assert.equal(Commends.commands[0].type, 'chat');
			assert.equal(Commends.commands[0].whitelist, null);
			assert.equal(Commends.commands[0].event, 'commends:get');
		});
		it('Contains settings for !setcommends', function() {
			assert.notEqual(Commends.commands, null);
			assert.equal(Commends.commands[1].name, 'setcommends');
			assert.equal(Commends.commands[1].type, 'whisper');
			assert.equal(Commends.commands[1].whitelist, true);
			assert.equal(Commends.commands[1].event, 'commends:set');
		});
	});
