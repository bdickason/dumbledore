...
describe('Commends', function() {
	...
	describe('Chat commands', function() {
		it('Contains settings for !status', function() {
			assert.notEqual(Status.commands, null);
			assert.equal(Status.commands[0].name, 'status');
			assert.equal(Status.commands[0].type, 'chat');
			assert.equal(Status.commands[0].whitelist, null);
			assert.equal(Status.commands[0].event, 'status:get');
		});
